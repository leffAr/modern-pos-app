...
class _DashboardScreenState extends State<DashboardScreen> {
            ...
            builder: (context, expSnapshot) {
              return StreamBuilder<List<DebtPayment>>(
                ...
                builder: (context, dpSnapshot) {
                  ...
                  final totalExpenseToday = todayExpenses.fold(0.0, (sum, e) => sum + e.amount);
                  final netProfitToday = finalTotalRevenueToday - totalExpenseToday;

              return SingleChildScrollView(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _buildWelcomeHeader(context, finalTotalRevenueToday),
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          _buildSummaryCards(
                            context: context, 
                            revenueToday: finalTotalRevenueToday, 
                            txTodayCount: todayResults.length, 
                            avgToday: averageToday, 
                            allTimeRev: finalAllTimeRevenue,
                            netProfitToday: netProfitToday,
                            expenseToday: totalExpenseToday,
                            piutangToday: finalTotalPiutangToday,
                            allTimePiutang: finalAllTimePiutang,
                          ),
                
                if (widget.userRole == 'Admin') ...[
                  const SizedBox(height: 32),
                  const Text('Performa Kasir (Hari Ini)', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF111827))),
                ...
                const SizedBox(height: 16),
                _buildRecentTransactions(recentTxs),
                    ],
                  ),
                ),
              ],
  ...
  Widget _buildWelcomeHeader(BuildContext context, double revenueToday) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            'Ikhtisar performa bisnis hari ini',
  ...
  Widget _buildSummaryCards({
    ...
    required int txTodayCount,
    required double avgToday,
    required double allTimeRev,
    required double netProfitToday,
    required double expenseToday,
    required double piutangToday,
    required double allTimePiutang,
  }) {
